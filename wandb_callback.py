...
class WandbCallback(BaseCallback):
    def __init__(self, vec_env, env_eval, wandb_run_id=None):
    ...
    def _init_callback(self):
        ...
        for i in range(self.vec_env.num_envs):
            ...
            self.vec_env.set_attr('last_epoch', self.model.i_update, indices=i)
            self.vec_env.set_attr('num_timesteps', self.model.num_timesteps, indices=i)
        for i in range(self.env_eval.num_envs):
            self.env_eval.set_attr('n_epoch', self.model.i_update, indices=i)
            self.env_eval.set_attr('last_epoch', self.model.i_update, indices=i)
            self.env_eval.set_attr('num_timesteps', self.model.num_timesteps, indices=i)
    ...
    def _on_training_end(self) -> None:
        ...
        if (self.model.num_timesteps % self._eval_step) < self.model.n_steps_total :
            ...
            # eval_video_path = (self._video_path / f'eval_town2_{self.model.num_timesteps}.mp4').as_posix()
            # avg_ep_stat, avg_route_completion, ep_events = self.evaluate_policy_town2(self.env_eval, self.model.policy, eval_video_path)
            # log to wandb
            # wandb.log({f'video/{self.model.num_timesteps}': wandb.Video(eval_video_path)},
            #           step=self.model.num_timesteps)
            # wandb.log(avg_ep_stat, step=self.model.num_timesteps)
    ...
    @staticmethod
    def evaluate_policy_town2(env, policy, video_path):
        policy = policy.eval()
        t0 = time.time()
        for i in range(env.num_envs):
            env.set_attr('eval_mode', True, indices=i)
        ...
        n_step = 0
        n_timeout = 0
        for route_id in range(env.get_attr('num_tasks', indices=0)[0]):
            for i in range(env.num_envs):
                env.env_method('set_task_idx', indices=i, task_idx=route_id)
            obs = env.reset()

            env_done = np.array([False]*env.num_envs)
            env_completed = np.array([False]*env.num_envs)
            while not np.any(env_done) and not np.any(env_completed):
                # not c_route and not done_ep:
                actions, values, log_probs, mu, sigma, _, fake_birdview = policy.forward(obs, deterministic=True)
                obs, reward, done, info = env.step(actions)
                ...
                _, c, h, w = fake_birdview.shape
                im_envs = []
                for i_env in range(env.num_envs):
                    im_render = np.zeros([h, w*3, c], dtype=np.uint8)
                    env_fake_birdview = fake_birdview[i_env]
                    env_fake_birdview = np.transpose(env_fake_birdview, [1, 2, 0])
                    im_render[:h, :w] = env_fake_birdview
                    env_render = env.env_method('render', indices=i_env)[0]
                    im_render[:h, w:] = env_render
                    im_envs.append(im_render)
                big_im = tile_images(im_envs)
                list_render.append(big_im)
                n_step += 1
                env_done |= done
                for i in range(env.num_envs):
                    env_completed[i] = info[i]['route_completion']['is_route_completed']

                for i in np.where(done)[0]:
                    if not info[i]['route_completion']['is_route_completed']:
                        ep_stat_buffer.append(info[i]['episode_stat'])
                    if not np.any(env_done) and not np.any(env_completed):
                        route_completion_buffer.append(info[i]['route_completion'])
                    ep_events[f'venv_{i}'].append(info[i]['episode_event'])
                    n_timeout += int(info[i]['timeout'])
        ...
        avg_ep_stat = WandbCallback.get_avg_ep_stat(ep_stat_buffer, prefix='eval_town2/')
        avg_route_completion = WandbCallback.get_avg_route_completion(route_completion_buffer, prefix='eval_town2/')
        avg_ep_stat['eval/eval_timeout'] = n_timeout
